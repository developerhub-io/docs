...
{% code-steps title="Mocking Default Landing Page" %}
Most projects should use the visual [Landing Page Designer](/support-center/landing-page), which already auto-generates cards from your content. This example is for [Custom HTML](/support-center/custom-landing-page) pages: it recreates a similar auto-generated cards layout by hand, using the `oncardschanged` event so the cards update whenever the contents of the docs change. The page consists of a "hero", which we won't go through since it's basic HTML, and then the autogenerated part.
...
{% step title="Listening to \"oncardschanged\"" codeId="eWJte" highlightLines="1-2,47-48" %}
To show the cards, we can listen to an [event](/support-center/developer-tools#landing-page-cards-generated) called `oncardschanged`. This event is dispatched when the reader navigates to the landing page. The event details contain the information needed to generate the layout.
{% /step %}

{% step title="Checking if we're on a landing page" codeId="eWJte" highlightLines="3-5" %}
...
{% step title="Generating cards" codeId="eWJte" highlightLines="6-20,44" %}
`event.detail.docs` is an array of objects representing the available [card generation strategy](/support-center/landing-page#card-generation-strategy). Each object (variable `card` here) has a `title` and a `link`. This represents a category in our documentation. We'll create a card of each one of them, then create an H4 which will contain the link to the title.
{% /step %}

{% step title="Generate links" codeId="eWJte" highlightLines="22-28" %}
...
{% step title="Finalising the UI" codeId="eWJte" highlightLines="30-43" %}
...
{% step title="Setting up Links" codeId="eWJte" highlightLines="46" %}
...
```html {% id="eWJte" title="Custom HEAD Tags" %}
<script>
