...
{% /step %}
{% step title="Generating cards" description="`event.detail.docs` is an array of objects representing the available [card generation strategy](/support-center/landing-page#card-generation-strategy). Each object (variable `card` here) has a `title` and a `link`. This represents a category in our documentation. We'll create a card of each one of them, then create an H4 which will contain the link to the title." codeId="eWJte" highlightLines="6-20,44" %}
{% /step %}
